...
matrix.return <- function(c, alpha, beta, delta.min, sigma){
        ...
        ############integral matrix################
        integral.matrixo <- matrix(0,6,5)
        for(i in 1:6){
                for(j in 1:5){
                        integral.matrixo[i,j] <- integrate(Vectorize(get(paste0("b",i))),ideal.pop.prime[1,j],ideal.pop.prime[2,j])$value
                }
        }
        ...
        avg.trteff <- sweep(integral.matrixo, 2, STATS = (ideal.pop.prime[2,] - ideal.pop.prime[1,]), FUN = "/")
        avg.trteff[, 5] <- 0
        
        ##########integral matrix with health care cost###########
        integral.matrix <-  sweep(integral.matrixo, 2, STATS = c * (ideal.pop.prime[2,] - ideal.pop.prime[1,]), FUN = "-")
        
        #######health system cost#########
        health.cost <- c * (ideal.pop.prime[2,] - ideal.pop.prime[1,])
        ...
        return(list(integral.matrix=integral.matrix,duration=duration,avg.trteff=avg.trteff,mean.dat=average.treat,ideal.pop.prime=ideal.pop.prime,power=power^2,
                    health.cost = health.cost, integral.matrixo = integral.matrixo))
}
